...
class History(tk.Frame):
    ...
    更改此function,對應到評論
    '''
    def restuaurant_comment_list(self, result=None):
    ...
    def restuaurant_history_list(self, result=None):
        cursor = self.connection.cursor()
        for widget in self.searchRestuaurantButtonFrame.winfo_children(): 
            widget.destroy()
        for i in range(len(result)):
            search_time = result[i].get('search_time', '')
            ...
            button_text = f'{store_name} \nGenre: {tag} {price_range} Rating: {rating}\n  {address}\n{search_time}' 
            tk.Button(self.searchRestuaurantButtonFrame, text=button_text, relief='solid', anchor='w', width=50, height=5, 
                      command=lambda id=restaurant_id, res=restaurant_info, rat=rating: self.to_the_restuarant(id, res, rat)).pack(pady=5, fill='x', expand=True)
        self.searchRestuaurantFrame.pack(fill='both', expand=True)
        cursor.close()
    ...
    def restuaurant_menu_list(self):
        ...
        cursor.execute(query, (self.restaurant_id,))
        result = cursor.fetchall()
        for widget in self.searchSpecificFrameMenuFrameMenuFrame.winfo_children():  #清空
            widget.destroy()
        for i in range(len(result)):
