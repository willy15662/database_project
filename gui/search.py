...
class Search(tk.Frame):
    def __init__(self, connection, parent, controller, UserInfo=None, Shortcut=None):
        ...
        ####
        #  self.UserInfo.info是一個dictionary, key: customer_id/owner_id, username, password, email, name
        # 可以用customer_id/owner_id來做query
        # e.g. self.UserInfo.info["customer_id"]; 另一種寫法: self.UserInfo.info.get("customer_id", "") 不會報error
        ...
        self.searchFrameSreachBarLabel = tk.Label(self.searchFrameSreachBar, text="Search the restaurant: ")
        self.searchFrameSreachBarLabel.grid(row=0, column=1, columnspan=2, pady=5)
        self.searchFrameSreachBarEntry = tk.Entry(self.searchFrameSreachBar)
        self.searchFrameSreachBarEntry.grid(row=0, column=2, columnspan=2, pady=5)
        self.searchFrameSreachBarButton = tk.Button(self.searchFrameSreachBar, text="search", command=lambda:self.search_the_restuaurant(), relief='solid')
        self.searchFrameSreachBarButton.grid(row=0, column=4)
        ...
        #分隔線
        self.searchSeparateLine = ttk.Separator(self, orient='horizontal')
        self.searchSeparateLine.pack(fill='x', pady=5)
    ...
    用來顯示搜尋結果的餐廳列表,
    更改此function,
    self.searchFrameSreachBarTypeCombobox、self.searchFrameSreachBarPriceCombobox、
    self.searchFrameSreachBarAddressCityCombobox、self.searchFrameSreachBarAddressDistrictCombobox為四個下拉式選單,
    添加餐廳列表需修改到對應的type、price range、address,
    '''
    def search_the_restuaurant(self):
    ...
    def add_to_shortcut(self):
        ...
        cursor.execute(query, (int(cid), keyword, self.restaurant_id,))
        self.connection.commit()
        cursor.close()
        ...
        # if len(self.categories[keyword] == 0): 
        #     self.categories[keyword] = []
        # self.categories[keyword].append(self.restaurant_id)
